...
AGunAlienWidget::AGunAlienWidget()
{
...
void AGunAlienWidget::BeginPlay()
{
	Super::BeginPlay();
...
void AGunAlienWidget::MouseInit()
{
	Super::MouseInit();
...
void AGunAlienWidget::MoveingUpdate(float DeltaTime)
{
...
void AGunAlienWidget::MouseDeathed()
{
	this->bIsShoot = false;
	//关闭发射组件
...
void AGunAlienWidget::InMapMeshe(ELineType CurLineType)
{
...
FVector AGunAlienWidget::GetBulletLauncherLocation()
{
	return FVector(this->LauncherBulletLocationComp->GetComponentLocation());
}
	...
	for (const FGunAlienCheckLineTraceBaseConfig& LineConfig : this->CheckLineTraceConfig)
	{
		...
		if (UGameSystemFunction::AddLineTrance(
			...
			[](UObject* GunAlienActor, AActor* CardActor) {
				if (IsValid(Cast<AGunAlien>(GunAlienActor)))
				{
					Cast<AGunAlien>(GunAlienActor)->SetCurrentAttackCard(
						Cast<ACardActor>(CardActor));
				}
				if (IsValid(Cast<AGunAlienWidget>(GunAlienActor)))
				{
					Cast<AGunAlienWidget>(GunAlienActor)->SetCurrentAttackCard(
						Cast<ACardActor>(CardActor));
				}
			}
		))
...
void AGunAlienWidget::SetShootEnable(bool Value)
{
...
bool AGunAlienWidget::GetbShootEnabled()
{
	return this->bShootEnabled;
}

bool AGunAlienWidget::GetbShoot()
{
	return this->bIsShoot;
}

void AGunAlienWidget::PlayMoveAnim()
{
	if (this->State != 0)
	{
		//走路状态
...
void AGunAlienWidget::PlayAttackAnim()
{
	if (!this->bShootEnabled)
	{
		return;
	...
	if (this->State != 1)
	{
		...
		UTrackEntry* Track = this->SetAnimation(0,
			...
			true);
		BINDANIMATION(Track, this, &AGunAlienWidget::OnAnimationPlayEnd);
		this->SetTrackEntry(Track);
	}
}

void AGunAlienWidget::PlayIdleAnim()
{
	if (!this->bShootEnabled)
	{
		return;
